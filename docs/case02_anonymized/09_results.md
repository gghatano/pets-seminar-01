...
- **置換**: 会員ID → 仮ID（対応表は残さない）
- **削除**: 氏名・電話番号／店舗ID・取引ID・担当者ID・商品ID・明細ID（不要・再識別リスク低減）
- **一般化**: 生年月日 → 年代7区分、住所 → 市区郡、利用日時 → 分単位
- **特異値処理**: 商品名 → カテゴリ、数量 → 上限5で丸め、金額 → 金額区分
...
!!! note "該当者が少ない組合せへの対応"
    市区郡・年代・性別まで細かく残すと、**該当者が数名以下の組合せ**が出ることがあります。そうした組合せは個人が浮きやすいため、次のように丸めを調整します。

    - **丸めの度合を粗くする**（例: 市区郡 → 都道府県、年代の区分をまとめる）
    - 該当者が極端に少ない値は **削除**する

    ポイントは、**該当者の人数に応じて丸めの度合を決める**こと。仮名加工（case01）が「照合しなければ特定できない」状態づくりだったのに対し、匿名加工は**丸めの度合を対象データに合わせて設計する**ぶん、ひと手間増えます。
